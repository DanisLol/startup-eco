...
import { getAuthenticatedParent, getParentChildDashboardData } from "@/lib/parent-dashboard";
...
export default async function ParentChildPage({
  ...
  if (demoMode && childId === "demo-child") {
    return <DashboardView data={getDemoParentDashboardData()} email="demo-parent@pebble.local" demoMode />;
  }

  const parent = await getAuthenticatedParent();
  if (!parent) redirect("/parent/login");
  const data = await getParentChildDashboardData(childId);
  if (!data) notFound();
